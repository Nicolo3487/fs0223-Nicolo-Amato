...
for(let c= 0; c < starWarsCharacters.length; c++){
  characters.push(starWarsCharacters[c].name)
}
...
/* ESERCIZIO 5
  Inserisci l'oggetto dei personaggi in "starWarsCharacters" nell'array corrispondente al 
  colore dei loro occhi nell'oggetto "eyeColor" precedentemente creato
  Utilizza uno switch statement per determinare in quale proprietà inserire il personaggio
  */

for(let c = 0; c < starWarsCharacters.length; c++){
   switch(starWarsCharacters[c].eye_color){
    case 'blue':
      eyeColor.blue.push(starWarsCharacters[c]);
      break;
    case 'yellow':
      eyeColor.yellow.push(starWarsCharacters[c]);
      break;
    case 'brown':
      eyeColor.brown.push(starWarsCharacters[c]);
      break;
    case 'red':
      eyeColor.red.push(starWarsCharacters[c]);
      break;
    case 'blue-gray':
      eyeColor.blueGray.push(starWarsCharacters[c]);
      break;
      default:
   }
}
...
while(y < starWarsCharacters.length){
  crew += parseInt(starWarsCharacters[y].mass);
  y++;
}
console.log(crew);
...
switch(true){
  ...
  case(crew == 500):
    ...
    break;
  case(crew > 700 && crew < 900):
    console.log('Warning: Load is over 700');
    break;
  case( crew >900 && crew < 1000):
